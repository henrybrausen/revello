...
int ABPlayer::alphabeta(int depth, int alpha, int beta)
{
    ...
    if (move_count == 1 && depth == 0) depth = 1;   // Search forced chains of moves up until their conclusion.
    
    if (depth <= 0 && m_board.last_move() != MOVE_PASS) {
		// We hit out search depth limit. Eval the board and return!
        int ret = 100*m_mobilityWeight*Board::num_moves(moves) + m_board.evaluate_territory()+m_board.stone_differential();
